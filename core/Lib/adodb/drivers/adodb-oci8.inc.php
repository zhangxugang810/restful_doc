...
function oci_lob_desc($type) {
    switch ($type) {
        case OCI_B_BFILE: $result = OCI_D_FILE;
            break;
        case OCI_B_CFILEE: $result = OCI_D_FILE;
            break;
        case OCI_B_CLOB: $result = OCI_D_LOB;
            break;
        case OCI_B_BLOB: $result = OCI_D_LOB;
            break;
        case OCI_B_ROWID: $result = OCI_D_ROWID;
            break;
...
";
    ...
    function UpdateBlob($table, $column, $val, $where, $blobtype = 'BLOB') {
        switch (strtoupper($blobtype)) {
            default: ADOConnection::outp("<b>UpdateBlob</b>: Unknown blobtype=$blobtype");
                return false;
            case 'BLOB': $type = OCI_B_BLOB;
                break;
            case 'CLOB': $type = OCI_B_CLOB;
                break;
        }
...
class ADORecordset_oci8 extends ADORecordSet {
    ...
    function ADORecordset_oci8($queryID, $mode = false) {
        ...
        switch ($mode) {
            case ADODB_FETCH_ASSOC:$this->fetchMode = OCI_ASSOC + OCI_RETURN_NULLS + OCI_RETURN_LOBS;
                break;
            case ADODB_FETCH_DEFAULT:
            case ADODB_FETCH_BOTH:$this->fetchMode = OCI_NUM + OCI_ASSOC + OCI_RETURN_NULLS + OCI_RETURN_LOBS;
                break;
            case ADODB_FETCH_NUM:
            default:
                $this->fetchMode = OCI_NUM + OCI_RETURN_NULLS + OCI_RETURN_LOBS;
                break;
        }
    ...
    function ADORecordSet_ext_oci8($queryID, $mode = false) {
        ...
        switch ($mode) {
            ...
            case ADODB_FETCH_BOTH:$this->fetchMode = OCI_NUM + OCI_ASSOC + OCI_RETURN_NULLS + OCI_RETURN_LOBS;
                break;
            case ADODB_FETCH_NUM:
            default: $this->fetchMode = OCI_NUM + OCI_RETURN_NULLS + OCI_RETURN_LOBS;
                break;
        }
        $this->adodbFetchMode = $mode;
        $this->_queryID = $queryID;
